...
#################gamlss
OxIndex <- read_delim(file="Data/OxCGRT_compact_national_v1.csv", delim=",")
FranceLine <- OxIndex[OxIndex$CountryName=="France", ]
FranceLine$Date
FranceLine$Date <- as.Date(as.character(FranceLine$Date), format ="%Y%m%d")
Selectedcolumns <- FranceLine[, c(6,7,9,11,13,15,17,53,54,55)]

Data0_merged = merge(Data0, Selectedcolumns, on="Date", all.x= T)
Data0_merged = Data0_merged %>% fill(everything(), .direction = "downup")

Data1_merged = merge(Data1, Selectedcolumns, on="Date", all.x= T)
Data1_merged = Data1_merged %>% fill(everything(), .direction = "downup")

Data0<-Data0_merged;Data1<-Data1_merged

sum(is.na(Data0_merged))
# Identify columns with spaces
columns_with_spaces <- grep(" ", colnames(Data0))

# Replace spaces with underscores
colnames(Data0)[columns_with_spaces] <- gsub(" ", "_", colnames(Data0)[columns_with_spaces])

# Identify columns with spaces
columns_with_spaces <- grep(" ", colnames(Data1))

# Replace spaces with underscores
colnames(Data1)[columns_with_spaces] <- gsub(" ", "_", colnames(Data1)[columns_with_spaces])

equation <- Net_demand ~ s(as.numeric(Date), bs='cr') + s(toy, bs='cc') + as.factor(WeekDays) + as.factor(BH) + s(Wind) + te(Net_demand.1, Net_demand.7) + te(toy, Temp)

gam_JC = gam( Net_demand ~ s(as.numeric(Date), k=3, bs='cr') 
              + s(toy,  bs='cr') 
              + as.factor(WeekDays)
              + as.factor(BH)
              + s(Wind, bs='cr')
              ...
              + te(Temp, toy)
              + te(Nebulosity, toy)
              + as.factor(C6M_Stay_at_home_requirements)
              + s(GovernmentResponseIndex_Average)
              + s(ContainmentHealthIndex_Average)
...
gam_JC.forecast <- predict(gam_JC, newdata = Data0[sel_b, ])
res <- Data0$Net_demand[sel_b] - gam_JC.forecast
quant <- qnorm(0.95, mean= mean(res), sd= sd(res))
pinball_loss(y=Data0$Net_demand[sel_b], gam_JC.forecast+quant, quant=0.95, output.vect=FALSE)

submit <- read_delim( file="Data/sample_submission.csv", delim=",")
submit$Net_demand <- predict(gam_JC, newdata = Data1)+quant
write.table(submit, file="Data/submission_GAM_JC_COVID.csv", quote=F, sep=",", dec='.',row.names = F)
